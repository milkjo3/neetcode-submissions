class Solution:
    def combinationSum(self, nums: List[int], target: int) -> List[List[int]]:
        # TC: O(n^(target / min(nums)))
        # We explore a recursion tree with branching factor n and
        # maximum depth target / min(nums).

        # SC: O(target / min(nums))
        # Maximum recursion depth and path length.
